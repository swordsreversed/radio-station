			...
			foreach($days as $day => $min) {
				...
				foreach($min as $shift) {
					...
					if(isset($shift['time']['real_start'])) {
							...
							$start_of_show_str = $day." ".$shift['time']['start_hour'].":".$shift['time']['start_min']." ".$shift['time']['start_meridian'];
							$strt_d = strtotime($start_of_show_str);
							$end_of_show_str = $day." ".$shift['time']['end_hour'].":".$shift['time']['end_min']." ".$shift['time']['end_meridian'];
							$end_d = strtotime($end_of_show_str);
							if (($current_ux_time > $strt_d) && ($current_ux_time < $end_d)) {
								$classes .= 'now-playing';
							} // end
							...
							$output .= '</span>';
							$output .= __($day, 'radio-station').'<br />';
							$output .= $shift['time']['real_start'];
							...
							$output .= $shift['time']['start_hour'].':'.$shift['time']['start_min'];
							if($timeformat == 12) {
								$output .= $shift['time']['start_meridian'];
							}
